          ...
          <AnimatePresence>
            {filteredCommands.map((cmd) => (
              <motion.div
                ...
                key={cmd.id}
                className="group bg-card rounded-2xl p-5 border border-border/50 shadow-sm hover:shadow-md hover:border-border transition-all duration-300 flex flex-col h-full relative overflow-hidden"
              >
                ...
                <div className="flex justify-between items-start mb-3 relative z-10">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-48 glass rounded-xl border-border/50">
                      <DropdownMenuItem onClick={() => handleEdit(cmd)} className="rounded-lg m-1 cursor-pointer">
                        <Edit2 className="w-4 h-4 mr-2" /> Edit
                      </DropdownMenuItem>
                      <DropdownMenuSeparator className="bg-border/50 mx-2" />
                      <DropdownMenuItem onClick={() => handleDelete(cmd.id)} className="rounded-lg m-1 text-destructive focus:bg-destructive/10 focus:text-destructive cursor-pointer">
