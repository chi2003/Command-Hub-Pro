...
export function RunChainDialog({ chain, open, onOpenChange }: RunChainDialogProps) {
  ...
  const handleCopySteps = async () => {
    const execSteps = chain.steps.slice(0, lastIndex).map(s => s.command).join('\n');
    try {
      await navigator.clipboard.writeText(execSteps);
      setCopiedSteps(true);
              ...
              {chain.steps.map((step, index) => {
                ...
                return (
                  <div key={step.id} className="relative">
                    <div className={`absolute -left-[31px] top-1 w-6 h-6 rounded-full bg-background border-2 flex items-center justify-center shadow-sm z-10 ${isLast ? 'border-amber-500/60' : 'border-primary shadow-primary/20'}`}>
                      {isLast
                      ...
                      <div className="rounded-xl border border-amber-500/25 bg-amber-500/5 overflow-hidden">
                        <div className="flex items-center justify-between px-4 py-2.5 border-b border-amber-500/20 bg-amber-500/10">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold text-amber-400">Paste to Terminal — Complete Before Running</span>
                          </div>
                          <button
                        ...
                        <div className="px-4 py-2.5 border-t border-amber-500/15">
                          <p className="text-[11px] text-amber-400/60 leading-relaxed">
                            After the steps above complete, paste this into your terminal. Edit any placeholders then press Enter.
                          </p>
                        </div>
                      </div>
                    ) : (
                      <div className="relative group">
                        <div className="bg-[#0D1117] rounded-xl border border-gray-800 overflow-hidden">
                          <div className="flex justify-between items-center px-4 py-2 bg-gray-900 border-b border-gray-800">
                            <span className="text-xs font-mono text-gray-400">{chain.shell === "powershell" ? "PowerShell" : "Command Prompt"}</span>
                            <button
                              onClick={() => handleCopySingle(step.command, step.id)}
        ...
        <div className="p-6 border-t border-border/50 bg-background/50 flex justify-end gap-3">
          ...
          {chain.steps.length > 1 && (
            <Button onClick={handleCopySteps} className="rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 shadow-lg shadow-primary/20 hover-elevate">
              ...
              {copiedSteps ? "Copied!" : "Copy Execute Steps"}
            </Button>
          )}
        </div>
      </DialogContent>
